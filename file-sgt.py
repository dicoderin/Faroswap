...
Final Version with Verified Transaction Parameters
"""
...
╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝
    """)
    print("Pharos Username Minter v7.7 | Final Version with Verified Transaction Parameters")
    print("=" * 60)
...
class PharosMultiMinter:
        ...
        try:
            ...
            if receipt:
                if receipt.status == 1:
                    ...
                    return {'secret': secret, 'commitment': commitment}
                else:
    ...
    def register_username(self, full_username, commit_data):
        """Register username after commitment with enhanced error handling"""
        try:
            self.registration_attempts += 1
            
            # Extract secret from commit data
            secret = commit_data['secret']
    ...
    def mint_username(self, max_attempts=7):
        ...
        for attempt in range(max_attempts):
            ...
            # Step 1: Make commitment (once per username)
            print("  🔐 Making commitment...")
            commit_data = self.make_commitment(full_username)
            if not commit_data:
            ...
            # Step 2: Register username (try up to 10 times with same commitment)
            print("  🔏 Registering username...")
            
            # Try register with retry logic (10 attempts with same commit data)
            for reg_attempt in range(10):  # Increased to 10 registration attempts per username
                print(f"  📝 Registration attempt {reg_attempt+1}/10...")
                tx_hash = self.register_username(full_username, commit_data)
                ...
                print(f"  ⚠️ Registration attempt {reg_attempt+1}/10 failed, retrying...")
