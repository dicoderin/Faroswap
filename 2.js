...
class PharosMultiMinter {
  constructor(privateKey, userAgent = null) {
    ...
    this.web3 = new Web3();
    const provider = new Web3.providers.HttpProvider(rpcUrl, { headers, timeout: 120000 });
  ...
  async estimateRegisterGas(fullUsername, owner, secret, fee) {
    try {
      const gasEstimate = await this.contract.methods.register(fullUsername, owner, secret).estimateGas({ from: this.account.address, value: fee });
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.log(" ⏳ Commit tx timeout, but continuing...");
          return secret;
        }
      } catch (e) {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.log(" ⏳ Register tx timeout");
          return txHash;
        }
      } catch (e) {
  ...
  async mintUsername(maxAttempts = 5) {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (txHash) {
        ...
        return { status: 'success', username: fullUsername, txHash: txHash, explorerUrl: explorerLink };
      }
...
function loadAccounts() {
  ...
  if (pk1 && pk1 !== '0x'.concat('0'.repeat(64))) {
    ...
    accounts.push({ name: 'Account 1 (iPhone 16 Pro Max)', privateKey: pk1, userAgent: ua1 });
  }
  ...
  if (pk2 && pk2 !== '0x'.concat('0'.repeat(64))) {
    ...
    accounts.push({ name: 'Account 2 (iPhone 15)', privateKey: pk2, userAgent: ua2 });
  }
  if (!accounts.length) {
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      try {
        ...
        if (result && result.status === 'success') {
          ...
          console.log(`\n⏳ Transaction pending after ${elapsed.toFixed(2)}s`);
        ...
        console.log(`\n⏳ Account processing time: ${elapsed.toFixed(2)} seconds`);
      } catch (e) {
