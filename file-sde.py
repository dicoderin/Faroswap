...
╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝
    """)
    print("Pharos Username Minter v7.6 | Professional Enhanced Commit-Register Process")
    print("=" * 60)
...
class PharosMultiMinter:
    def __init__(self, private_key, user_agent=None, max_retries=5):
        ...
        # Initialize cached values with accurate values based on real transactions
        # Updated based on actual transaction info provided
        self.cached_minting_fee = Web3.to_wei(0.0032, 'ether')  # Updated from 0.01 to 0.0032 PHRS
        ...
        # Method IDs based on successful transactions
        self.commit_method_id = "0xf14fcbc8"  # From transaction 0x71183046630d5a9c3370e5b2ac3ea55cae020e650ca4fdb554ecd95c93d74c1b
        self.register_method_id = "0x5a2ee019"  # From transaction 0xf63723c7ba348e26095411b335d254b13e67a1eff9271fb34d689c383f073915
        ...
        # Use SocialScan as block explorer
        self.block_explorer_url = os.getenv(
            'EXPLORER_BASE_URL', 
    ...
    def get_minting_fee(self, use_cache=True):
        ...
        def get_fee_rpc():
            try:
                ...
                abi_signature = "0x3ccfd60b"  # Function signature for mintingFee()
    ...
    def get_gas_price(self):
        ...
        # Method 4: Use hardcoded fallback for Pharos testnet (1 Gwei based on screenshot)
        fallback_gas = Web3.to_wei(1, 'gwei')  # Updated from 20 to 1 based on screenshot
        gas_prices.append(fallback_gas)
        print(f"  💡 Method 4 fallback: {Web3.from_wei(fallback_gas, 'gwei')} gwei")
        ...
        if gas_prices:
            ...
            default_gas = Web3.to_wei(1, 'gwei')  # Updated from 30 to 1
    ...
    def estimate_register_gas(self, full_username, owner, secret, fee):
        ...
        if gas_estimate is not None:
            ...
            default_gas = 600000  # Higher default to ensure success
    ...
    def make_commitment(self, full_username):
        ...
        try:
            ...
            print(f"  🔍 View transaction: {self.block_explorer_url}{tx_hash.hex()}")
    ...
    def register_username(self, full_username, secret):
        ...
        try:
            ...
            # Use higher gas limit for safety instead of estimation
            gas_limit = 600000  # Fixed high value to ensure success
            print(f"  ⛽ Using fixed gas limit for registration: {gas_limit}")
    ...
    def mint_username(self, max_attempts=7):
        ...
        for attempt in range(max_attempts):
            ...
            for reg_attempt in range(3):  # 3 registration attempts per username
                ...
                if tx_hash:
                    explorer_link = f"{self.block_explorer_url}{tx_hash.hex()}"
                ...
                # Try different gas price/limit for next attempt
                time.sleep(3)  # Wait longer between registration attempts
            
            print("  ❌ All registration attempts failed for this username, trying another")
            time.sleep(2)  # Longer delay between mint attempts
