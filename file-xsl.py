...
Deteksi gas fee commit dan register secara otomatis
"""
...
def inject_poa_middleware(w3):
    """Injeksi POA middleware dengan kompatibilitas"""
    try:
        # For Web3.py v6+
        try:
...
class PharosMultiMinter:
    def __init__(self, private_key, user_agent=None):
        ...
        # Initialize Web3 dengan custom session dan timeout
    ...
    def generate_commitment(self, name, secret):
        """Generate commitment hash untuk username"""
        try:
    ...
    def get_min_commitment_age(self):
        """Get minimum commitment age dalam detik"""
        try:
            return self.contract.functions.minCommitmentAge().call()
        except Exception as e:
            print(f"⚠️ Failed to get min commitment age: {str(e)}")
            return 60  # Default ke 60 detik
    
    def get_minting_fee(self):
        """Get minting fee dalam wei"""
        try:
            return self.contract.functions.mintingFee().call()
        except Exception as e:
            print(f"⚠️ Failed to get minting fee: {str(e)}")
            # Return default fee jika gagal
            return Web3.to_wei(0.01, 'ether')

    def get_balance(self):
        """Get account balance dalam PHRS"""
        try:
            balance = self.w3.eth.get_balance(self.account.address)
            return Web3.from_wei(balance, 'ether')
        except Exception as e:
            print(f"⚠️ Failed to get balance: {str(e)}")
            return 0.0

    def is_username_available(self, full_username):
        """
        Cek apakah username tersedia
        Returns True jika tersedia, False jika sudah diambil
        """
        try:
            # Check availability
            ...
            if "already registered" in error_msg.lower() or "exists" in error_msg.lower():
                return False
            print(f"⚠️ 'available' function error: {error_msg}")
            return False
        except Exception as e:
            print(f"⚠️ 'available' function error: {str(e)}")
            return False
    
    def get_gas_price(self):
        """
        Deteksi gas price otomatis dengan multiple fallbacks
        Returns optimal gas price dalam wei
        """
        gas_prices = []
    ...
    def estimate_register_gas(self, full_username, owner, secret, fee):
        ...
        try:
            ...
            # Add 50% buffer untuk register (lebih tinggi karena kompleksitas)
            gas_limit = int(gas_estimate * 1.5)
            print(f"  ⛽ Register gas estimate: {gas_estimate} (using {gas_limit})")
            return gas_limit
        except Exception as e:
            print(f"  ⚠️ Register gas estimation failed: {str(e)}")
            # Return default yang lebih tinggi untuk register
            return 450000
    
    def sign_and_send_transaction(self, tx):
        """Sign dan send transaction dengan kompatibilitas berbagai versi"""
        try:
            # Sign transaction
            ...
            print(f"  ❌ Transaction signing/sending failed: {str(e)}")
            return None
    
    def make_commitment(self, full_username):
        """Create dan send commitment transaction"""
        # Generate secret dan commitment
        ...
        try:
            ...
            # Estimasi gas untuk commit (auto)
            ...
            try:
                ...
                print("  ⏱️ Commit transaction timeout, but continuing...")
    ...
    def register_username(self, full_username, secret):
        ...
        try:
            ...
            # Estimasi gas untuk register (auto)
            ...
            try:
                ...
                print("  ⏱️ Register transaction timeout")
    ...
    def mint_username(self, max_attempts=5):
        """
        Mint username baru dengan two-step commit-register process
        Akan mencoba hingga max_attempts kali untuk menemukan nama yang tersedia
        """
...
# =============================================
# FUNGSI LOAD ACCOUNTS
# =============================================
def load_accounts():
    """Load konfigurasi account dari environment"""
    accounts = []
